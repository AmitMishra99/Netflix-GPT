...
const SearchPage = () => {
  ...
  const handleSearchSubmit = async (e) => {
    e.preventDefault();
    const response = await api.post("/gpt/search", { query: getQuery });
    const gptMovies = response.data.movies;
    console.log(gptMovies);
    const promiseArray = gptMovies.map((movie) => searchMovie(movie));
    const data = await Promise.all(promiseArray);
    dispatch(addGPTMovies({ moviesTitle: gptMovies, moviesRes: data }));
    console.log(data);
  };
    ...
    <div className="min-h-screen bg-black text-white pt-24 sm:pt-28 px-4 sm:px-8 md:px-16">
      ...
      <div className="max-w-3xl mx-auto pt-6">
        ...
        >
          ...
          <button
            type="submit"
            className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl text-sm sm:text-base shadow-lg transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer transform hover:scale-[1.02]"
          >
            <span>Search</span>
          </button>
        </form>
